...
describe("movement functions", function()
  ...
  it("can move to the left and again", function()
    ...
    assert.are.same({"system", {"tmux send-keys -t :-1 Up C-m"}}, vim.api._called()[4])
    assert.are.same({"system", {"tmux select-window -t :-1"}}, vim.api._called()[5])
  end)
  it("can move to the right and again", function()
    tmux.mv_to_window_right_and_again()
    assert.are.same({"system", {"tmux send-keys -t :+1 Up C-m"}}, vim.api._called()[6])
    assert.are.same({"system", {"tmux select-window -t :+1"}}, vim.api._called()[7])
  end)
  it("can move to the alternate window and again", function()
    value = random_string("alt_window")
    stub.stubber.var("l42_tmux_alternate_window", value)
    tmux.mv_to_alternate_window_and_again()
    assert.are.same({"system", {"tmux send-keys -t :=" .. value .. " Up C-m"}}, vim.api._called()[8])
    assert.are.same({"system", {"tmux select-window -t :=" .. value}}, vim.api._called()[9])
  end)
end)
